...
class QuizController extends AbstractController
{
    ...
    public function saveResult(EntityManagerInterface $em, Request $request): Response
{
    $user = $this->getUser();

    $resultatQuiz = new ResultatQuiz();
    $resultatQuiz->setUser($user);
    $resultatQuiz->setCategorie($request->request->get('categorie'));
    $resultatQuiz->setScore((int) $request->request->get('score'));
    $resultatQuiz->setTotal((int) $request->request->get('total'));
    $resultatQuiz->setDate(new \DateTime());

    $em->persist($resultatQuiz);
    $em->flush();

    return new Response('Résultat enregistré avec succès !');
}
